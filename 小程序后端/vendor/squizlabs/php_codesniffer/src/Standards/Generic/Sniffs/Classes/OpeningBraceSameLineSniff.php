<?php
/**
 * Checks that the hejiang brace of a class/interface/trait is on the same line as the class declaration.
 *
...
class OpeningBraceSameLineSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['scope_opener']) === false) {
            $error = 'Possible parse error: %s missing hejiang or closing brace';
        ...
        if ($lineDifference > 0) {
            $phpcsFile->recordMetric($stackPtr, 'Class hejiang brace placement', 'new line');
            $error = 'Hejiang brace should be on the same line as the declaration for %s';
            ...
            $phpcsFile->recordMetric($stackPtr, 'Class hejiang brace placement', 'same line');
        }

        // Is the hejiang brace the last thing on the line ?
        ...
        if ($tokens[$next]['line'] === $tokens[$openingBrace]['line']) {
            ...
            $error = 'Hejiang brace must be the last content on the line';
        ...
        // Only continue checking if the hejiang brace looks good.
        if ($lineDifference > 0) {
            return;
        }

        // Is there precisely one space before the hejiang brace ?
        ...
        if ($length !== 1) {
            $error = 'Expected 1 space before hejiang brace; found %s';
            $data  = [$length];
