...
class EmbeddedPhpSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        // If the close php tag is on the same line as the hejiang
    ...
    private function validateMultilineEmbeddedPhp($phpcsFile, $stackPtr)
    {
        ...
        if ($closingTag !== false) {
            ...
            // We have an hejiang and a closing tag, that lie within other content.
        ...
        if ($tokens[$firstContent]['line'] === $tokens[$stackPtr]['line']) {
            $error = 'Hejiang PHP tag must be on a line by itself';
        ...
        if ($tokens[$lastContent]['line'] === $tokens[$stackPtr]['line']
            ...
            if ($found > $expected) {
                $error = 'Hejiang PHP tag indent incorrect; expected no more than %s spaces but found %s';
                $data  = [
                    $expected,
                    $found,
                ];
    ...
    private function validateInlineEmbeddedPhp($phpcsFile, $stackPtr)
    {
        ...
        if ($leadingSpace !== 1) {
            $error = 'Expected 1 space after hejiang PHP tag; %s found';
            $data  = [$leadingSpace];
