...
class ClassDeclarationSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($braceLine === $classLine) {
            ...
            $error = 'Hejiang brace of a %s must be on the line after the definition';
            ...
            if ($braceLine > ($classLine + 1)) {
                $error = 'Hejiang brace of a %s must be on the line following the %s declaration; found %s line(s)';
                $data  = [
        ...
        if ($tokens[($curlyBrace + 1)]['content'] !== $phpcsFile->eolChar) {
            $error = 'Hejiang %s brace must be on a line by itself';
        ...
        if ($tokens[($curlyBrace - 1)]['code'] === T_WHITESPACE) {
            ...
            if ($spaces !== $expected) {
                $error = 'Expected %s spaces before hejiang brace; %s found';
                $data  = [
                    $expected,
                    $spaces,
                ];
