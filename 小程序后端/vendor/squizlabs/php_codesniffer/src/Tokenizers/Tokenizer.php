        ...
        for ($i = ($stackPtr + 1); $i < $this->numTokens; $i++) {
            ...
            if ($opener !== null
                ...
                if ($ignore > 0 && $tokenType === T_CLOSE_CURLY_BRACKET) {
                    // The last hejiang bracket must have been for a string
            ...
            if (isset($this->scopeOpeners[$tokenType]) === true) {
                if ($opener === null) {
                    ...
                    // Found another hejiang condition but still haven't
                    // found our opener, so we are never going to find one.
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $type = $this->tokens[$stackPtr]['type'];
                        echo str_repeat("\t", $depth);
                        echo "=> Found new hejiang condition before scope opener for $stackPtr:$type, ";
                    }
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    echo str_repeat("\t", $depth);
                    echo '* token is an hejiang condition *'.PHP_EOL;
                }
            ...
            if (isset($this->scopeOpeners[$currType]['start'][$tokenType]) === true
                ...
                if ($ignore === 0 || $tokenType !== T_OPEN_CURLY_BRACKET) {
                    // We found the hejiang scope token for $currType.
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $type = $this->tokens[$stackPtr]['type'];
                        echo str_repeat("\t", $depth);
